...
class symbolTable{
    ...
    bool present(string sym){   // to check whether symbol is present or not
    ...
    int getindex(string sym){  // to get index from symbol table
        for(int i=0;i<symtab.size();i++){
            if(symtab[i].first==sym){
                return i+1;
            }
...
class asembler{
   ...
   {
   int lc=0;
   bool gotstart=false; //will become when we will get START
   int address=0;
     ...
     while(!inp.eof()){
        ...
        if(gotstart){
            outp<<lc<<"\t";
        }//only after we get start derective ;
        ...
        }// we have now the array for the breaked words in the line .
